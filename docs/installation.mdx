...
```bash
python ./main.py --groups groups.yaml --external-ip YOUR_EXTERNAL_IP
```
...
python ./main.py --from-db --external-ip YOUR_EXTERNAL_IP

# Start a specific group from database
python ./main.py --from-db --group beta --external-ip YOUR_EXTERNAL_IP
```
...
When `main.py` runs without CLI flags (e.g., `python main.py` without `--groups`
