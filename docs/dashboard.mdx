...
Manage users across groups:

| Feature | Details |
|---------|---------|
| List | All users with group, email, workspace path |
| Search | Filter by username, group, or email |
| Create | Add user to a group with optional email |
| Edit | Update username, email, group assignment |
| Delete | Remove user and their records |
| Launch instance | Start a sandbox for a specific user with PVC workspace |
| Stop instance | Stop a user's running sandbox |

### Workspaces

Overview of workspace volumes and mount status:

| Feature | Details |
|---------|---------|
| Volume list | Docker named volumes (`thon-workspace-*`, `thon-storage-*`) |
| Mount status | Whether workspace is currently attached to a sandbox |
| Group/user mapping | Which group and user each volume belongs to |
...
| File | Purpose |
|------|---------|
| `dashboard/streamlit_app.py` | Main app with 7 pages and sidebar navigation |
