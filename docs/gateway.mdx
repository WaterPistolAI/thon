...
Uniform rate limits across all models for each consumer. Every model request
counts against the same token quota.

### per-model

Different concurrency and token limits per model. Configured via the
`gateway.model_concurrency` list in `thon.yaml`:

```yaml
gateway:
  rate_limit_scope: "per-model"
  model_concurrency:
...
Each entry defines per-model limits that override the global defaults when
`rate_limit_scope` is set to `per-model`.

## Concurrency Control

Each consumer gets a `limit-conn` plugin that limits the number of concurrent
requests. This prevents a single user from overwhelming the LLM backend with
parallel requests.

| Setting | Default | Description |
|---------|---------|-------------|
| `gateway.concurrency_limit` | `1` | Max concurrent requests per consumer |
| `GATEWAY_CONCURRENCY_LIMIT` | `1` | Environment variable override |

When using Redis, concurrency counters are shared across gateway instances.

## Kilo Code Integration

When the gateway is enabled, `main.py` generates a gateway-aware `kilo.jsonc`
...
In per-group mode, all users in the same group receive the same `kilo.jsonc`
with the shared group API key.

When `--no-embedding` is used during setup, the `indexing` section is omitted
from the generated `kilo.jsonc`.
