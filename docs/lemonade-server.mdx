...
6. Creates `kilo.jsonc` for Kilo Code (with indexing config)
...
- `kilo.jsonc` omits the `indexing` section
...
This rewrites `recipe_options.json` with new `ctx_size` and `-np` values based
on the user count, then reloads models so new settings take effect immediately.

The rescaling uses the `lemonade.llamacpp` settings from `thon.yaml` to construct
the `llamacpp_args` string. If no `thon.yaml` exists, sensible defaults are used.
...
| `batch_size` | `8192` | Batch size (`-b`) |
| `ubatch_size` | `8192` | Micro-batch size (`-ub`) |
| `split_mode` | `""` | Multi-GPU split mode (`--split-mode`) |
| `main_gpu` | `-1` | Primary GPU index (`--main-gpu`) |
| `cpu_moe` | `false` | Offload MoE to CPU (`--cpu-moe`) |
| `n_cpu_moe` | `0` | Number of CPU MoE experts (`--n-cpu-moe`) |
| `min_p` | `0.0` | Minimum P sampling (`--min-p`) |
| `presence_penalty` | `0.0` | Presence penalty (`--presence-penalty`) |

Additional args always included: `-to 3600 --temp 1.0 --top-k 64 --top-p 0.95
--repeat-penalty 1.0 --no-webui --threads-http -1 --threads -1 -np <num_users>`

## Chat Models

The `lemonade.chat_models` list in `thon.yaml` allows registering multiple models
for selection in Kilo Code:
...
Each entry generates a model in `user_models.json` and a corresponding entry in
...
Creates `kilo.jsonc` with chat model, experimental flags, and semantic indexing:
